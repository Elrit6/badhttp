...
void Response::returnContent(const std::string& content) {
...
void Server::handleClient(SOCKET& clientSocket) {
	char buffer[1024];
	uint16_t bytesReceived = recv(clientSocket, buffer, sizeof(buffer), 0);
	if (bytesReceived > 0) {
...
void Server::run() {
	while (true) {
		...
		int32_t clientAddressSize = sizeof(clientAddress);
